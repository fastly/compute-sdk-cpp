...
cmake-example name=default-example:
    cmake -S ./examples -B {{ build-dir }}/examples -DWASI_SDK={{ wasi-sdk }} -DEXAMPLE_NAME={{ name }} -DENABLE_LTO=ON -DCMAKE_BUILD_TYPE={{ type }} -DCMAKE_TOOLCHAIN_FILE={{ wasi-sdk }}/share/cmake/wasi-sdk-p1.cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=1
...
cmake:
    cmake -S . -B {{ build-dir }} -DWASI_SDK={{ wasi-sdk }} -DENABLE_LTO=ON -DCMAKE_BUILD_TYPE={{ type }} -DCMAKE_TOOLCHAIN_FILE={{ wasi-sdk }}/share/cmake/wasi-sdk-p1.cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=1
